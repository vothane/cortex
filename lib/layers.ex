...
defmodule Dense do
  ...
  def backward_propogate(dense_layer, accum_grad) do 
    ...
    if get(dense_layer, :trainable) do
      ...
      grad_w = Nx.dot(Nx.transpose(input), accum_grad)
      grad_bias = Nx.sum(accum_grad, axes: [0])
    ...
    Nx.dot(accum_grad, Nx.transpose(w))
  end
  
  @impl Layer
  def init!(dense_layer, optimizer) do
    put(dense_layer, :weights, Nx.random_normal({elem(get(dense_layer, :shape_input), 1), get(dense_layer, :n)}, 0.0, 0.5))
...
defmodule Flatten do
  ...
  def forward_propogate(flatten_layer, t) do
    {rows, cols} = shape_t = Nx.shape(t)
    put(flatten_layer, :prev_shape, shape_t)
    Nx.reshape(t, {1, rows * cols})
  end
  
  @impl Layer
  def backward_propogate(flatten_layer, accum_grad) do
    {rows, cols} = get(flatten_layer, :prev_shape)
    Nx.reshape(accum_grad, {rows, cols})
  end
  
  @impl Layer
  def output_shape(flatten_layer) do
    {rows, cols} = Nx.shape(get(flatten_layer, :input_shape))
    {rows * cols, 1}
  end
...
defmodule Dropout do
  ...
  @impl Layer
  def forward_propogate(dropout_layer, t) do
    ...
    if train? do
      ...
      masked = new_tensor({rows, cols}, masker)
  ...
  def backward_propogate(dropout_layer, accum_grad) do
    Nx.multiply(accum_grad, get(dropout_layer, :mask))
  end
